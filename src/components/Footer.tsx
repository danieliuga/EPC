...
import { socialUrls } from '../data/content'
...
export default function Footer() {
  ...
  const links = [
    ...
    { label: 'Dona', href: '/contacto/donar' },
    { label: 'Contacto', href: '/contacto' },
  ]
  const socials = [
    { label: 'YouTube', href: socialUrls.youtube, icon: (
    ...
    { label: 'Telegram', href: socialUrls.telegram, icon: (
    ...
    { label: 'Instagram', href: socialUrls.instagram, icon: (
    ...
    { label: 'TikTok', href: socialUrls.tiktok, icon: (
    ...
    { label: 'X', href: socialUrls.x, icon: (
    ...
    <footer className="bg-ink text-white">
      <div className="container-app py-12">
        <div className="flex items-end justify-between">
          <h3 className="font-display text-2xl md:text-3xl tracking-tight text-white">EL FUTURO NOS PERTENECE</h3>
          <div className="hidden md:flex items-center gap-4">
            <span className="text-sm text-white/50">Síguenos:</span>
            {socials.map((s) => (
              <a key={s.label} href={s.href} aria-label={s.label} className="text-white/50 hover:text-primary transition-colors">
                {s.icon}
              </a>
            ))}
          </div>
        </div>
        <div className="mt-6 grid grid-cols-2 gap-6 text-sm md:grid-cols-3 lg:grid-cols-5">
          {links.map((l) => (
            <NavLink key={l.label} to={l.href} className="text-white/50 hover:text-white transition-colors">{l.label}</NavLink>
          ))}
        </div>
        <hr className="my-6 border-white/10" />
        <div className="flex flex-col items-start justify-between gap-4 text-sm md:flex-row">
          <p className="text-white/40">© {year} EPC — Estudiants Pel Canvi</p>
